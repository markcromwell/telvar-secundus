...
## then enemy turn. Wires Use Item: Healing Herb restores 10 HP (clamped), updates
## the player HP bar, then passes the turn.

const MELEE_RANGE_TILES: int = 1
const FLEE_SUCCESS_CHANCE: float = 0.6
const HEALING_HERB_HP: int = 10

## Consumable tracked here (CombatManager is frozen for other phases).
var _healing_herbs_remaining: int = 1
...
func _try_wire_use_item_button() -> void:
	var scene_root: Node = get_tree().current_scene
	if scene_root == null:
		return
	var use_btn: Node = scene_root.find_child("UseItem", true, false)
	if use_btn is Button:
		var b: Button = use_btn as Button
		if not b.pressed.is_connected(_on_use_item_pressed):
			b.pressed.connect(_on_use_item_pressed)
...
func _on_use_item_pressed() -> void:
	...
	if CombatManager.enemy_hp <= 0:
		return
	if _healing_herbs_remaining <= 0:
		_spawn_item_message_float("No healing herbs.")
		return
	if CombatManager.player_hp >= CombatManager.player_max_hp:
		_spawn_item_message_float("HP is already full.")
		return

	_healing_herbs_remaining -= 1
	CombatManager.player_hp = mini(CombatManager.player_max_hp, CombatManager.player_hp + HEALING_HERB_HP)
	_refresh_player_hp_bar()
	_spawn_heal_float(HEALING_HERB_HP)
	CombatManager.begin_enemy_turn()
...
func _spawn_heal_float(amount: int) -> void:
	...
	label.text = "+%d" % amount
	label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	label.add_theme_color_override("font_color", Color(0.35, 0.95, 0.45))
	label.add_theme_color_override("font_outline_color", Color(0, 0, 0))
	label.add_theme_constant_override("outline_size", 5)
...
func _spawn_item_message_float(message: String) -> void:
	var scene_root: Node = get_tree().current_scene
	if scene_root == null:
		return
	var host: Control = scene_root.get_node_or_null("Root") as Control
